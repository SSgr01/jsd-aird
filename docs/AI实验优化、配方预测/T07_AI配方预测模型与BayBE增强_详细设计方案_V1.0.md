...
> 日期：2026-09-03  
> 状态：T07-A、T07-A.1、T07-A.2、T07-A.3、T07-A.4、T07-A.5已完成；T07-B待实施  
...
- **T07-A算法预开发**：A完成独立服务与GP/RF闭环，A.1完成可重复X→多Y调用，
  ...
  固化、稳定性摘要与批量评分基准。
- **T07-B 生产集成适配**：等T05/T06合并后接入`ExperimentAnalysisView`、
  T06案例基线/规则、PostgreSQL异步任务、对象存储授权和客户研究任务。
...
首个生产版本人工激活。快照新增至少10条或增长20%后可在周日02:00重建；新模型
综合NMAE改善至少5%且核心目标不退化才自动替换。连续3次模型调用失败时回退上
一目标版本；最近10条正式反馈误差超过验证误差1.5倍时暂停自动使用并告警。
