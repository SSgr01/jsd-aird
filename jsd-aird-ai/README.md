...
内部接口仅保留`formula-model.v2`：

- `POST /internal/v2/snapshots/validate`
- `POST /internal/v2/snapshots/validation-folds`
- `POST /internal/v2/models/train`
- `POST /internal/v2/models/score`
- `POST /internal/v2/models/recommend`
- `GET /internal/v2/health/live` 和 `GET /internal/v2/health/ready`

`/internal/v1/*`已经移除并返回404。V2契约要求显式的Y定义、输入方案、材料字典、
预处理、训练策略和版本哈希；不提供旧契约兼容转换。
