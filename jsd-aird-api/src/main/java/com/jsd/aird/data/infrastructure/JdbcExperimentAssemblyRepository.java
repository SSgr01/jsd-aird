...
public class JdbcExperimentAssemblyRepository implements ExperimentAssemblyRepository {
    ...
    @Override public void upsertPlan(UUID org, UUID job, PlanRow p, UUID actor) {
        jdbc.update("""
            INSERT INTO data.import_experiment_link(id,organization_id,import_job_id,recognition_job_id,
              experiment_boundary_id,assembly_key,parent_assembly_key,source_identity,source_identity_type,
              source_record_keys_jsonb,shared_context_record_keys_jsonb,plan_hash,content_hash,status,
              conflicts_jsonb,warnings_jsonb,sample_boundary_ids_jsonb,logical_sample_keys_jsonb,
              source_group_keys_jsonb,created_by)
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            ...
            """, UUID.randomUUID(),org,job,job,p.assemblyKey(),p.assemblyKey(),p.parentAssemblyKey(),
                p.sourceIdentity(),p.sourceIdentityType(),pg(p.sourceRecordKeys()),pg(p.sharedContextRecordKeys()),
                p.planHash(),p.contentHash(),p.status(),pg(p.conflicts()),pg(p.warnings()),
                pg(json.createArrayNode().add(p.assemblyKey()+"/sample-1")),
                pg(json.createArrayNode().add("SOURCE:"+job+":"+p.assemblyKey()+"/sample-1")),
                pg(p.sourceRecordKeys().isEmpty()?json.createArrayNode().add(p.assemblyKey()):p.sourceRecordKeys()),actor);
    }
