...
/** Cooperative deadline used by asynchronous CPU-heavy jobs. */
public final class JobDeadline {
    ...
    public static void check() {
        if (Thread.currentThread().isInterrupted()) {
            throw new DeadlineExceededException("异步任务已被中断");
        }
        var deadline = CURRENT.get();
        if (deadline != null && !Instant.now().isBefore(deadline)) {
            throw new DeadlineExceededException("异步任务执行超时");
        }
    }
    ...
    public static final class Scope implements AutoCloseable {
        ...
        private Scope(Instant previous) {
