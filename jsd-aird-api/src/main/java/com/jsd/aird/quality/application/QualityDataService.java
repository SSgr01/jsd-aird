...
public class QualityDataService {
    ...
    @Transactional public QualityDataStore.UploadView upload(UploadInput input) {
        ...
        if (input.sha256() != null && !input.sha256().isBlank()) {
            var existing = repository.findUploadByHash(a.organizationId(), input.sha256());
        ...
        try (var stored = storage.open(a.organizationId(), input.fileId())) {
            sourceParser.parse(def, data, stored);
        } catch (Exception e) {
        ...
        var recordId=repository.insertDraft(a.organizationId(),a.userId(),def.id(),category.id(),no,data,input);
    ...
    public PageResponse<QualityDataStore.UploadView> uploads(String keyword,String status,UUID projectId,int page,int size) {
        var a=ActorContext.required();
        return repository.uploads(a.organizationId(),a.role(),keyword,status,projectId,Math.max(1,page),Math.min(100,Math.max(1,size)));
    }
    ...
    private void validateDraft(QualityDataDefinitions.Type def, JsonNode data) {
        ...
        for(var f:def.fields()) {
            ...
            if (f.required() && value.isEmpty()) throw new ApiException(ApiErrorCode.BAD_REQUEST,f.label()+"不能为空");
