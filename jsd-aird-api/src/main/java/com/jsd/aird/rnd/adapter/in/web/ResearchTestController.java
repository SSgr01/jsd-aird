...
public class ResearchTestController {
    ...
    @PostMapping("/comprehensive-reports/uploads") public ApiResponse<?> upload(@Valid @RequestBody UploadRequest r){return ok(service.upload(new ResearchTestService.UploadCommand(r.fileId,r.originalName,r.contentType,r.size,r.sha256,r.category,r.ownerName,r.visibility,r.projectId,r.stageId,r.taskId)));}
    @GetMapping("/comprehensive-reports/uploads") public ApiResponse<?> uploads(@RequestParam(required=false)String keyword,@RequestParam(defaultValue="1")int page,@RequestParam(defaultValue="10")int size){return ok(service.uploads(keyword,page,size));}
    @PostMapping("/comprehensive-reports/uploads/{id}/retry") public ApiResponse<?> retryUpload(@PathVariable UUID id){return ok(service.retryUpload(id));}
    @DeleteMapping("/comprehensive-reports/uploads/{id}") public ApiResponse<?> deleteUpload(@PathVariable UUID id){service.deleteUpload(id);return ok(null);}
    ...
    @PutMapping({"/comprehensive-reports/{id}/rename","/test-standards/{id}/rename"}) public ApiResponse<?> rename(@PathVariable UUID id,@Valid @RequestBody RenameRequest r){return ok(service.rename(id,new ResearchTestService.RenameCommand(r.revision(),r.name(),r.projectId(),r.stageId(),r.taskId())));}
    ...
    public record RenameRequest(@NotNull Long revision,@NotBlank @Size(max=300) String name,UUID projectId,UUID stageId,UUID taskId){}
    public record UploadRequest(@NotNull UUID fileId,@NotBlank String originalName,@NotBlank String contentType,@Positive long size,String sha256,String category,String ownerName,String visibility,UUID projectId,UUID stageId,UUID taskId){}
