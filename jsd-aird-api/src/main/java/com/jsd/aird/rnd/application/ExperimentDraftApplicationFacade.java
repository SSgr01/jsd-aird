...
public class ExperimentDraftApplicationFacade implements ExperimentDraftFacade {
    ...
    public ImportedDraft createResearchDraft(ResearchDraftCommand command) {
        ...
        var hasTemplate = command.templateVersionId() != null;
        if (hasTemplate && (command.templateSnapshotHash() == null || command.templateSnapshotHash().isBlank()
                || command.templateSnapshot() == null || !command.templateSnapshot().isObject()
                || command.templateSnapshot().isEmpty())) {
        ...
        var result = repository.create(new ExperimentRepository.Create(
                ...
                command.templateVersionId(), hasTemplate ? command.templateSnapshotHash().strip() : null,
                hasTemplate ? command.templateSnapshot().deepCopy() : json.createObjectNode(),
                editModel, actor.userId(), actor.username()
        ));
        return new ImportedDraft(result.id(), versionId, result.experimentNo(), result.title());
    }
