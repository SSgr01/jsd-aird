...
public class JdbcResearchTestRepository implements ResearchTestRepository {
    ...
    @Override @Transactional public Detail rename(UUID org,UUID id,String name,String businessNo,String ownerName,LocalDate date,String category,String scope,LocalDate effectiveFrom,LocalDate effectiveTo,UUID projectId,UUID stageId,UUID taskId,long lockVersion,UUID actor,String actorName){
        var old=required(org,id);
        if (businessNo!=null && exists(org,old.summary().recordType(),businessNo,id)) conflict("编号 "+businessNo+" 已存在");
        var n=jdbc.update("UPDATE rnd.research_test_record SET name=?,business_no=COALESCE(?,business_no),owner_name=COALESCE(?,owner_name),business_date=COALESCE(?,business_date),category=COALESCE(?,category),applicable_scope=COALESCE(?,applicable_scope),project_id=?,stage_id=?,task_id=?,lock_version=lock_version+1,updated_by=?,updated_at=now() WHERE organization_id=? AND id=? AND lock_version=?",
                name.trim(),businessNo,ownerName,date,category,scope,projectId,stageId,taskId,actor,org,id,lockVersion);
        lock(n);
        if (old.summary().recordType()==Type.STANDARD) {
            jdbc.update("UPDATE rnd.research_test_version SET effective_from=?,effective_to=? WHERE organization_id=? AND id=?",
                    effectiveFrom,effectiveTo,org,old.versionId());
        }
    ...
    @Override @Transactional public Upload addUpload(UUID org,UUID actor,UUID recordId,UUID fileId,String name,String ct,long size,String sha){var record=required(org,recordId);var id=UUID.randomUUID();try{jdbc.update("INSERT INTO rnd.research_test_upload(id,organization_id,record_id,file_id,original_name,content_type,file_size,sha256,created_by) VALUES(?,?,?,?,?,?,?,?,?)",id,org,recordId,fileId,name,ct,size,sha,actor);}catch(DuplicateKeyException e){throw new ApiException(ApiErrorCode.RESOURCE_CONFLICT,"相同内容的文件已上传");}return uploads(org,record.summary().recordType(),name,1,100).items().stream().filter(x->x.id().equals(id)).findFirst().orElseThrow();}
    ...
    @Override public PageResponse<Upload> uploads(UUID org,Type type,String keyword,int page,int size){var w=" WHERE u.organization_id=? AND u.deleted=false AND r.record_type=?";var a=new ArrayList<Object>();a.add(org);a.add(type.name());if(text(keyword)){w+=" AND lower(u.original_name) LIKE ?";a.add("%"+keyword.toLowerCase()+"%");}var joins=" FROM rnd.research_test_upload u JOIN rnd.research_test_record r ON r.id=u.record_id";long total=jdbc.queryForObject("SELECT count(*)"+joins+w,Long.class,a.toArray());a.add(size);a.add((page-1)*size);var items=jdbc.query("SELECT u.*,r.category,r.visibility,p.name project_name,s.name stage_name,t.name task_name"+joins+" LEFT JOIN mdm.project p ON p.id=r.project_id AND p.deleted=false LEFT JOIN mdm.project_stage s ON s.id=r.stage_id AND s.deleted=false LEFT JOIN mdm.project_task t ON t.id=r.task_id AND t.deleted=false"+w+" ORDER BY u.created_at DESC LIMIT ? OFFSET ?",this::upload,a.toArray());return new PageResponse<>(items,page,size,total,(total+size-1)/size);}
