...
public class TemplateImportContractCompiler {
    ...
    public CompiledContract compile(JsonNode layoutSummary, JsonNode schema, JsonNode mappings) {
        ...
        var experimentTemplate = isExperimentConfiguration(experimentConfiguration);
    ...
    private boolean isExperimentConfiguration(JsonNode configuration) {
        if (!"EXPERIMENT_DATA".equalsIgnoreCase(configuration.path("templateUsage").asText(""))) {
            return false;
        }
        var projections = configuration.path("listProjections");
        if (projections.isArray() && !projections.isEmpty()) {
            return true;
        }
        var identities = configuration.path("identities");
        if (!identities.isArray() || identities.isEmpty()) return true;
        for (var identity : identities) {
            if (!"BATCH_NO".equals(identity.path("identityType").asText(""))) return true;
        }
        return false;
    }
    ...
    private ArrayNode components(JsonNode mappings, JsonNode schema, boolean includeExperimentSemantics) {
        ...
        for (var mapping : mappings) {
            ...
            // Older recognition runs stored experiment list projections against
            // the stable physical block id while the mapping table groups rows
            // by the root binding id. Keep both identifiers on the compiled
            // component so those immutable drafts remain executable.
            var physicalBlockId = firstNonBlank(mapping, "blockId");
            if (physicalBlockId.isBlank()) physicalBlockId = mapping.path("diagnostic").path("blockId").asText("");
            if (physicalBlockId.isBlank()) {
                physicalBlockId = firstNonBlank(locator, "blockId", "regionId");
            }
            if (!physicalBlockId.isBlank()) {
                if (component.path("blockId").asText("").isBlank()) component.put("blockId", physicalBlockId);
                var aliases = component.withArray("componentAliases");
                var exists = false;
                for (var alias : aliases) {
                    if (physicalBlockId.equals(alias.asText(""))) {
                        exists = true;
                        break;
                    }
                }
                if (!exists) aliases.add(physicalBlockId);
            }
    ...
    private ObjectNode findComponent(ArrayNode components, String componentId) {
        for (var component : components) {
            if (componentId.equals(component.path("componentId").asText("") )
                    || componentId.equals(component.path("blockId").asText(""))
                    || containsText(component.path("componentAliases"), componentId)) {
                return (ObjectNode) component;
            }
        }
        return null;
    }
    ...
    private boolean containsText(JsonNode values, String expected) {
        ...
        for (var value : values) if (expected.equals(value.asText(""))) return true;
        return false;
    }
