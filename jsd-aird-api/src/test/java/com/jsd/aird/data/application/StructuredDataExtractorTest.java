...
class StructuredDataExtractorTest {
    ...
    @Test
    void singleFileExperimentUsesRightMostPopulatedResultColumnAfterStaticMethodColumn() {
        var sheet = sheet("report", List.of(
                List.of("指标", "", "", "", ""),
                List.of("外观", "", "目测", "透明液体", ""),
                List.of("光泽", "", "60度角光泽计", "49.4 GU", "")));
        var fields = List.of(
                binding("report", "appearance", "appearance", "外观", "C2:E2", "COLUMN", false),
                binding("report", "gloss", "gloss", "光泽", "C3:E3", "COLUMN", false));
        var contract = mapper.createObjectNode().put("templateUsage", "EXPERIMENT_DATA");
        contract.putObject("experimentImport").put("recordMode", "SINGLE_FILE");

        var result = extractor.extract(sheet, definitions("appearance", "gloss"), fields,
                1, 3, 9, contract).orElseThrow();

        assertThat(result.rows()).hasSize(1);
        assertThat(result.rows().getFirst().rawValues().path("b_appearance").asText())
                .isEqualTo("透明液体");
        assertThat(result.rows().getFirst().rawValues().path("b_gloss").asText())
                .isEqualTo("49.4 GU");
    }
