...
export function CategoryCardGrid({ categories, activeId, addLabel = '新增分类', onSelect, onCreate, onRename, onDelete }: CategoryCardGridProps) {
  return (
      ...
      {categories.map((category) => (
        ...
        >
          ...
          <div className="catalog-category-copy">
            ...
            <Typography.Text strong>{category.count}</Typography.Text><Typography.Text type="secondary"> 条记录</Typography.Text>
          </div>
          {category.editable && (onRename || onDelete) && (
            <Space className="catalog-category-actions" size={2} onClick={(event) => event.stopPropagation()}>
