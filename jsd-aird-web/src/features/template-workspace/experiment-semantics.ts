...
/**
 * Only expose experiment semantics when the saved configuration contains an
 * explicit experiment identity or a structured experiment matrix. A batch
 * number by itself is common in ordinary production records and must not turn
 * those templates into experiment templates in the workspace UI.
 */
export function isExperimentTemplate(configuration: ExperimentImportConfiguration) {
  if (configuration.templateUsage !== 'EXPERIMENT_DATA') return false;
  if (configuration.listProjections.length > 0) return true;
  if (configuration.identities.length === 0) return true;
  return configuration.identities.some((identity) => identity.identityType !== 'BATCH_NO');
}
