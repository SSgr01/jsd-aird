...
export function DataViewPage() {
  ...
  const cards = useMemo<CatalogCategoryCard[]>(() => [
    ...
    ...categories.map((item) => ({ id: item.id, name: item.name, count: item.assetCount, description: item.description, icon: <DatabaseOutlined />, tone: 'blue' as const, editable: true })),
  ], [categories]);
  ...
  const saveCategory = async (value: CategoryEditorValue) => {
    ...
    try {
      ...
      if (editor?.mode === 'NEW') await dataApi.createCategory({ name: value.name.trim(), targetDataType: value.targetDataType as DataType | undefined, description });
  ...
  const resolveAssetFile = async (asset: DataAsset): Promise<FilePreviewDescriptor> => {
    const source = await dataApi.resolveSourceFile(asset.id, asset.currentRevisionId);
    if (!source) throw new Error('该数据资产没有可用的原始文件');
    return { fileName: source.fileName, load: () => dataApi.sourceBlob(source.fileId) };
  };
  const previewAsset = async (asset: DataAsset) => {
    try { setPreviewFile(await resolveAssetFile(asset)); }
    catch (error) { void message.error(error instanceof Error ? error.message : '原始数据文件加载失败'); }
  };
  const downloadAsset = async (asset: DataAsset) => {
    try { await downloadPreviewFile(await resolveAssetFile(asset)); void message.success('原文件下载已开始'); }
  ...
  return <div className="business-page">
    ...
    <CatalogListPanel title={cards.find((item) => item.id === categoryId)?.name || '数据资产'} count={page.total} filters={<Space wrap><Input.Search allowClear placeholder="搜索资产编号或名称" value={keyword} onChange={(event) => { setKeyword(event.target.value); setPage((value) => ({ ...value, current: 1 })); }} onSearch={() => void load()} /><Select allowClear placeholder="全部状态" value={status} onChange={(value) => { setStatus(value); setPage((current) => ({ ...current, current: 1 })); }} options={Object.entries(statusLabels).map(([value, item]) => ({ value, label: item[0] }))} /><Button icon={<ReloadOutlined />} onClick={() => void load()}>刷新</Button></Space>} actions={<><Typography.Text type="secondary">已选 {selected.length} 项</Typography.Text><Button onClick={() => setSelected(items.map((item) => item.id))} disabled={!items.length}>选择当前结果</Button><Button onClick={() => setSelected([])} disabled={!selected.length}>清空选择</Button><Button type="primary" icon={<DownloadOutlined />} disabled={!selected.length} onClick={openExport}>批量导出文件</Button></>} loading={loading}>
      <Table rowKey="id" rowSelection={{ selectedRowKeys: selected, onChange: (keys) => setSelected(keys.map(String)) }} dataSource={items} pagination={{ current: page.current, pageSize: page.pageSize, total: page.total, showSizeChanger: true, onChange: (current, pageSize) => setPage({ current, pageSize, total: page.total }) }} locale={{ emptyText: <Empty description="当前分类暂无正式数据资产" /> }} onRow={(record) => ({ onDoubleClick: () => navigate(`/data/assets/${record.id}`) })} columns={[{ title: '资产名称', dataIndex: 'displayName', render: (value: string | undefined) => <Space><DatabaseOutlined /><Typography.Text strong>{value || '未命名资产'}</Typography.Text></Space> }, { title: '数据类型', dataIndex: 'targetDataType', render: (value: DataType) => dataTypeOptions.find((option) => option.value === value)?.label || value }, { title: '资产编码', dataIndex: 'assetKey' }, { title: '分类', dataIndex: 'categoryName', render: (value?: string) => value || '未分类' }, { title: '状态', dataIndex: 'status', render: (value: string) => <Tag color={statusLabels[value]?.[1]}>{statusLabels[value]?.[0] || value}</Tag> }, { title: '最近更新', dataIndex: 'updatedAt', render: (value: string) => new Date(value).toLocaleString('zh-CN') }, { title: '操作', width: 390, render: (_, record) => <Space wrap><Button type="link" icon={<EyeOutlined />} onClick={() => void previewAsset(record)}>预览</Button><Button type="link" icon={<DownloadOutlined />} onClick={() => void downloadAsset(record)}>下载</Button><Button type="link" onClick={() => navigate(`/data/assets/${record.id}`)}>查看详情</Button><Dropdown trigger={['click']} menu={{ items: categories.filter((category) => !category.targetDataType || category.targetDataType === record.targetDataType).map((category) => ({ key: category.id, label: category.name, onClick: () => void moveAsset(record, category.id) })) }}><Button type="link">移动分类</Button></Dropdown></Space> }]} />
    </CatalogListPanel>
    <CategoryEditorModal open={Boolean(editor)} title={editor?.mode === 'NEW' ? '新增数据分类' : '编辑数据分类'} initialValue={editor?.item ? { name: editor.item.name, description: editor.item.description, targetDataType: editor.item.targetDataType } : { name: '' }} targetDataTypeOptions={dataTypeOptions} confirmLoading={saving} onCancel={() => setEditor(undefined)} onSubmit={(value) => void saveCategory(value)} />
