...
export function FormulaPredictionPage() {
  ...
  useEffect(() => {
    formulaDesignApi.context().then((initial) => {
      ...
      setContext(initial)
      const remembered = new URLSearchParams(window.location.search).get('runId') ?? window.sessionStorage?.getItem('r08.formulaRunId') ?? null
  ...
  const materialLabel = (material: any, index: number) => material?.name || knownMaterialLabels[material?.materialId] || material?.code || `材料 ${index + 1}`
