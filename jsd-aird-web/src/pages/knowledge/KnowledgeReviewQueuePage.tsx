  ...
  return <div className="business-page">
    ...
    <Card className="content-card">
      <Table rowKey="versionId" loading={loading} dataSource={items} locale={{ emptyText: <Empty description="当前没有待处理的知识版本" /> }} columns={[
        ...
        { title: '解析', dataIndex: 'processingStatus', render: (value: string) => <Tag color={value === 'READY' ? 'success' : value === 'FAILED' ? 'error' : 'processing'}>{value === 'READY' ? '解析完成' : value === 'FAILED' ? '解析失败' : '解析中'}</Tag> },
        { title: '校对', render: (_: unknown, item: ReviewQueueItem) => { const state = reviewQueueStatus(item); return <Tag color={state.color}>{state.label}</Tag>; } },
        { title: '最近更新', dataIndex: 'updatedAt', render: (value: string) => new Date(value).toLocaleString('zh-CN') },
        { title: '操作', width: 140, render: (_: unknown, item: ReviewQueueItem) => <Button type={item.reviewRevisionStatus === 'FAILED' ? 'default' : 'primary'} onClick={() => navigate(`/knowledge/review/${item.documentId}/${item.versionId}`)}>{item.reviewRevisionStatus === 'FAILED' ? '查看详情' : '开始校对'}</Button> },
      ]} />
    </Card>
  </div>;
}
